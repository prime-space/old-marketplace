...
class db
{
	...
	function __construct($show_error=0, $db = null){
		global $CONFIG;
		$db = null === $db ? $CONFIG['mysql']['db'] : $CONFIG['mysql']["{$db}_db"];
		$this->connect($CONFIG['mysql']['user'], $CONFIG['mysql']['pass'], $db, $CONFIG['mysql']['host']);
	}
	...
	function connect($db_user, $db_pass, $db_name, $db_location = 'localhost', $show_error=1)
	{
		...
		mysqli_query($this->db_id, "SET time_zone = '+3:00'");
		mysqli_query($this->db_id, "SET lc_time_names = 'ru_RU'");
		mysqli_query($this->db_id, "SET sql_mode = 'ERROR_FOR_DIVISION_BY_ZERO,NO_AUTO_CREATE_USER,NO_ENGINE_SUBSTITUTION'");
		return true;
	}
...
//										 'num'   => (count($this->query_list) + 1));
	...
	function super_query_value($query){
		$request = $this->super_query($query);
		if($request)return current($request);
		return false;
	}
	...
	function safesql($source,$html = false,$nl2br = false, $real = true){
		if($html){
			$source = htmlentities($source,ENT_QUOTES,'UTF-8');
			$source = str_replace('&shy;','',$source);
		}
		if($nl2br)$source = nl2br($source,false);
		
		if($real)$source = mysqli_real_escape_string($this->db_id, $source);
