<?php
	...
	if($data['period'] == 'day'){
		$request = $bd->read("SELECT sales, sales_profit, ad_graphic, ad_text, ad FROM report_money WHERE isMerchant = 0 ORDER BY date DESC LIMIT 30");
		for($i=29;$i>=0;$i--){
	...
	if($data['period'] == 'month'){
		...
		for($i=1;$i<=$count_month;$i++){
			...
			$request[] = "SELECT SUM(sales), SUM(sales_profit), SUM(ad_graphic), SUM(ad_text), SUM(ad) FROM report_money WHERE isMerchant = 0 AND date > '".$this_date."' AND date <= '".$this_date_end."'";
		}
