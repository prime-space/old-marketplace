<?php
  ...
  $return['sale'] = mysql_result($request,0,4);
	if(mysql_result($request,0,8)){
		$picture = mysql_result($request,0,9) ? mysql_result($request,0,10).mysql_result($request,0,9) : 'picture/'.mysql_result($request,0,10).'productshow.jpg';
		$fullviewimage = mysql_result($request,0,9) ? mysql_result($request,0,10).mysql_result($request,0,9) : 'picture/'.mysql_result($request,0,10).'fullview.jpg';
	}else{
		$picture = 'stylisation/images/no_img.png';
		$fullviewimage = $picture;
	}
	$return['picture'] = '<a onclick="primearea.fullviewimage(\''.$siteAddr.$fullviewimage.'\');"><img src="'.$siteAddr.$picture.'"></a>';
