...
function backlightHtml(txt)
{
	...
	.replace(/ ((align|valign|src|id|style|width|body|cellspacing|cellpadding|border|height|href|content|http-equiv|type|rel|class|target))=/g, function(str1,str2)
	{
		var l = attr.length; 
		attr.push(str2); 			
		return '~~~ATTR'+l+'~~~';  	
	})	
	.replace(/(&lt;\/?(table|td|tr|span|div|html|header|body|br|img|a|strong|meta|title|head|pre|code|link|li|ul|ol|p|h1|h2|h3|h4|h5|script|b)(&gt;)?)/g,'<span class="code-tags">$1</span>')	
...
function backlightPHP(txt)
{
	...
	.replace(/(\s|\!|\(|^|\t)(function|array|foreach|class|static|empty|esle|if|break|return|exit|isset|while|explode|date|echo|as|file_get_contents|preg_split)(\s|\(|$|;)/g,'$1<span class="code-operator">$2</span>$3')
